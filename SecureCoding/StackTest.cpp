/* StackTest.cpp defines test-methods for the 
 *  dynamically allocated (array) Stack operations
 * Student Name: Huy Tran
 */
...
void StackTest::testConstructor() {
	...
	try {
		Stack s1(1);
		cerr << "\nConstructed stack of size 1\n";
		if (s1.getMyCapacity() <= 0) {
			throw CustomStackException("testConstructor()", "size must be positive");
		}
...
void StackTest::testPushPeekTopAndIsFull() {
	...
	try {
		if (s2.isFull()) {
			...
			s2.push(44);
			cerr << "\npush() worked on a full stack (size 4)\n";
		}
	} catch (CustomStackException& se) {
		cout << " 2 " << flush;
	}
...
void StackTest::testPop() {
	...
	try {
		s3.pop();
		cerr << "\npop() worked on non-empty stack (size 1)\n";
		if (s3.isEmpty()) {
    ...
    try {
            s3.peekTop();
            cerr << "\npeekTop() worked on non-empty stack (size 1)\n";
			if (s3.isEmpty()) {
	...
	try {
		s4.pop();
		cerr << "\npop() worked on non-empty stack (size 3)\n";
		if (s4.isEmpty()) {
			throw CustomStackException("pop()", "Stack is empty");
		}
	} catch (CustomStackException& se) {
    ...
    try {
            s4.peekTop();
            cerr << "\npeekTop() worked on non-empty stack (size 3)\n";
			if (s4.isEmpty()) {
				throw CustomStackException("peekTop()", "Stack is empty");
			}
    } catch (CustomStackException& se) {
